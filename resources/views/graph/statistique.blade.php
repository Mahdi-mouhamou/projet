...
@section('content')
    ...
    @if ($value === 'couteSismiquephase')
        ...
        <script>
            var ctx = document.getElementById('myChart2D3D');
            var labels = {{ Js::from($dataSismique['phase']) }};
            var data2D = {{ Js::from($dataSismique['2D']) }};
            var data3D = {{ Js::from($dataSismique['3D']) }};
            var myChart2D3D = new Chart(ctx, {
                type: "bar",
    ...
    @if ($value === 'couteSismiqueAnnee')
        ...
        <script src="{{ asset('chart.js/chart.js') }}"></script>
        <script>
            var ctx = document.getElementById('myChartAnne');
            var labels = {{ Js::from($data['year']) }};
            var data2D = {{ Js::from($data['coute2D']) }};
            var data3D = {{ Js::from($data['coute3D']) }};
            var myChartAnne = new Chart(ctx, {
                type: "line",
                data: {
                    labels: labels,
                    datasets: [{
    ...
    @if ($value === 'couteMagnetiquePhase')
        <div class="charts">
            ...
            <script>
                var ctx = document.getElementById('myChartAnne');
                var labels = {{ Js::from($dataMagnetique['phase']) }};
                var dataMagnetique = {{ Js::from($dataMagnetique['coute']) }};
                var myChartAnne = new Chart(ctx, {
                    type: "bar",
                    data: {
                        labels: labels,
                        datasets: [{
                            data: dataMagnetique,
                            backgroundColor: ["red", "lightblue", "lightgreen"],
                            borderColor: "red",
    ...
    @endif
    @if ($value === 'perimetreAnne')
        <div class="charts">
            <div class="chart">
                <h2 style="color: #000000;text-align: center ">
                    Nombre de perimetre par annee
                </h2>
                <canvas id="myChartAnne" role="img" aria-label="chart"></canvas>
            </div>
        </div>
        ...
        <script>
            var ctx = document.getElementById('myChartAnne');
            var labels = {{ Js::from($data['year']) }};
            var data = {{ Js::from($data['nbPerimetre']) }};
            var myChartAnne = new Chart(ctx, {
                type: "line",
                data: {
                    labels: labels,
