                            ...
                            @foreach ($dataTypeRows as $row)
                                ...
                                <div class="form-group @if ($row->type == 'hidden') hidden @endif col-md-{{ $display_options->width ?? 12 }} {{ $errors->has($row->field) ? 'has-error' : '' }}"
                                    ...
                                    @elseif ($row->type == 'relationship')
                                        ...
                                        @if ($add)
                                        {{-- {{dd($sismique->id)}} --}}
                                        <input class="form-control" type="text" name="realisation_sismique_id" readonly
                                        value="{{$sismique->id}}"> 
                                        @endif
                                       
                 
                                           {{-- @if ('add')
                                            <input class="form-control" type="text" name="valide" readonly
                                            value="{{session('status')}}"> 
                                           @endif  --}}
                                        {{-- <select  class="form-control select2-ajax" name="realisation_sismique_id"
                                            data-get-items-route="http://projet.test/admin/finance-sismiques/relation"
                                            data-get-items-field="finance_sismique_belongsto_realisation_sismique_relationship"
                                            data-method="add">
