...
class RealisationSismiqueController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    {
        // dd($request->TypeSismique);
        if($request->TypeSismique!="Methode potentiel"){
           if($request->Operation!="Etude"){
            $slug = $this->getSlug($request);
        $dataType = Voyager::model('DataType')->where('slug', '=', $slug)->first();
        // Check permission
        $this->authorize('add', app($dataType->model_name));
        ...
        if (!$request->has('_tagging')) {
            ...
            return redirect()->route("voyager.realisation-sismiques.create")->with([
                'message'    => "sismique 2D et 3D est uniquement en Acquisition ou traitement ou retraitement",
                'alert-type' => 'error',
            ]);   
        }
            ...
            if($request->Operation==="Acquisition"||$request->Operation==="Etude"){
                $slug = $this->getSlug($request);
                ...
                return redirect()->route("voyager.realisation-sismiques.create")->with([
                    'message'    => "Methode potentiel est uniquement en Acquisition ou etude",
                    'alert-type' => 'error',
                ]); 
            }
        }
