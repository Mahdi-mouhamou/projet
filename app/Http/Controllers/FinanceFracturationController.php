...
class FinanceFracturationController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $fin=FinanceFracturation::find($id);
    
        $Fin_id= $fin->fracturation_id;


        $prog = FinanceFracturation::where('id','=',$id)->first();
        $prog->valide = "non";
        $prog->save();
        $slug = $this->getSlug($request);
        ...
        $db=DB::table('finance_fracturations')
              ->select('*')
              ->where('id','=',$id)
              ->update(['fracturation_id' => $Fin_id]);
        // Delete Images
    ...
    {
        $fract = Fracturation::where('id', \request("id"))->first();
        $slug = $this->getSlug($request);
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable','fract'));
    }
    ...
    {
        $Real = Fracturation::where('id','=',$request->fracturation_id)->first();
        $Real->valide = "oui";
        $Real->save();
        $slug = $this->getSlug($request);
        ...
        // $data->update(["fracturation_id"=>$request->valide]);
        $mytime = Carbon::now()->format('Y/m/d H:i:s');
        $str=$dataType->model_name;
        $ch=str_split($str, 11);
        $chaine=$ch[1];
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $fin=DB::table('finance_fracturations')->select('fracturation_id')->where('id','=',$id)->first();
        // dd($fin);
        $Real = Fracturation::where('id','=',$fin->fracturation_id)->first();
        // dd($Real);
        $Real->valide = "non";
        $Real->save();
    ...
    {        
            $fin = FinanceFracturation::where('id', \request("id"))->first();
            $fin->valide = "oui";
            $mytime = Carbon::now();
            ...
            return redirect(route('voyager.finance-fracturations.index'));
    }  
}
