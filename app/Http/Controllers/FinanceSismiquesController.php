...
class FinanceSismiquesController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    public function edit(Request $request, $id)
    {
        ...
        //  dd($dataTypeContent->realisation_sismique_id);
    ...
    {
        $fin=FinanceSismique::find($id);
    
        $Fin_id= $fin->realisation_sismique_id;
        // dd($Fin_id);
        
        $prog = FinanceSismique::where('id','=',$id)->first();
        $prog->valide = "non";
        $prog->save();

        // $financeS=FinanceSismique::find($id)->update(['realisation_sismique_id'=>$dataTypeContent->realisation_sismique_id]);
        $slug = $this->getSlug($request);
 ...
 // dd($data);
        ...
        // $data->update(['realisation_sismique_id'=>$dataTypeContent->realisation_sismique_id]);
        ...
        $db=DB::table('finance_sismiques')
              ->select('*')
              ->where('id','=',$id)
              ->update(['realisation_sismique_id' => $Fin_id]);
        // Delete Images
    ...
    {
        // dd($request->realisation_sismique_id);
        $sismique = RealisationSismique::where('id', \request("id"))->first();
        // dd($sismique->id);
        $slug = $this->getSlug($request);
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable','sismique'));
    }
    ...
    {
        // dd($request->realisation_sismique_id);
        $Real = RealisationSismique::where('id','=',$request->realisation_sismique_id)->first();
        // dd($Real);
        $Real->valide = "oui";
        $Real->save();
        $slug = $this->getSlug($request);
    ...
    {
        // dd($id);
        $fin=DB::table('finance_sismiques')->select('realisation_sismique_id')->where('id','=',$id)->first();
        // // dd($fin->puit_id);
        $Real = RealisationSismique::where('id','=',$fin->realisation_sismique_id)->first();
        // // dd($Real);
        $Real->valide = "non";
        $Real->save();
    ...
    {        
            $fin = FinanceSismique::where('id', \request("id"))->first();
            $fin->valide = "oui";
            $mytime = Carbon::now();
            $fin->dateValidation=$mytime;
            $fin->save();
            $mytime = Carbon::now()->format('Y/m/d H:i:s');
            ...
            return redirect(route('voyager.finance-sismiques.index'));
    }   
}
