...
class FinanceGgController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $fin=FinanceGg::find($id);
    
        $Fin_id= $fin->Realisation_Gg_id;

        $prog = FinanceGg::where('id','=',$id)->first();
        $prog->valide = "non";
        $prog->save();
        $slug = $this->getSlug($request);
        ...
        $db=DB::table('finance_ggs')
              ->select('*')
              ->where('id','=',$id)
              ->update(['Realisation_Gg_id' => $Fin_id]);
    ...
    {
        $gg = RealisationGg::where('id', \request("id"))->first();
        $slug = $this->getSlug($request);
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable','gg'));
    }
    ...
    {
        $Real = RealisationGg::where('id','=',$request->Realisation_Gg_id)->first();
        $Real->Valide = "oui";
        $Real->save();
        $slug = $this->getSlug($request);
        ...
        // $data->update(["realisation_gg_id"=>$request->valide]);
        $mytime = Carbon::now()->format('Y/m/d H:i:s');
        $str=$dataType->model_name;
        $ch=str_split($str, 11);
        $chaine=$ch[1];
    ...
    {
        $fin=DB::table('finance_ggs')->select('Realisation_Gg_id')->where('id','=',$id)->first();
        // dd($fin->puit_id);
        $Real = RealisationGg::where('id','=',$fin->Realisation_Gg_id)->first();
        // dd($Real);
        $Real->Valide = "non";
        $Real->save();
        $slug = $this->getSlug($request);
    ...
    {        
            $fin = FinanceGg::where('id', \request("id"))->first();
            $fin->valide = "oui";
            $mytime = Carbon::now();
            ...
            return redirect(route('voyager.finance-ggs.index'));
    }   
}
