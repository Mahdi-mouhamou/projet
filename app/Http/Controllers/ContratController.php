...
class ContratController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $valide=Contrat::where('id',$data->id)->update(["valide"=>'non']);
        // dd($valide);
    ...
    public function create(Request $request)
    {
        ...
        $peri = Perimetre::where('id', \request("id"))->first();
        // dd($peri);
        $slug = $this->getSlug($request);
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable','peri'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $valideC= Perimetre::where('id',$data->perimetre_id)->update(["valideC"=>'oui']);
        // $data->update([]);
        $mytime = Carbon::now()->format('Y/m/d H:i:s');
        $str=$dataType->model_name;
        $ch=str_split($str, 11);
    ...
    public function destroy(Request $request, $id)
    {
        ...
        foreach ($ids as $id) {
           ...
           $chaine=$ch[1];
            ...
            $valideC= Perimetre::where('id',$data->perimetre_id)->update(["valideC"=>'non']);
            $avenant = Avenant::where('contrat_id', $id)->get();
            if (is_null($avenant)) {
            } else {
            ...
            $programme = Programme::where('contrat_id', $id)->get();
            ...
            foreach ($programme as $programme) {
                ...
                $pg = Programme::where('id', $programme->id)->delete();
             }
            }
            $realisationS=RealisationSismique::where('contrat_id',$id)->get();
        ...
        $realisationG=RealisationGg::where('contrat_id',$id)->get();
    ...
    {        
            $fin = Contrat::where('id', \request("id"))->first();
            $fin->valide = "oui";
            $mytime = Carbon::now()->format('Y/m/d H:i:s');
            ...
            return redirect(route('voyager.contrats.index'));
    }   
    // protected function AddContrat(){
    // $peri = Perimetre::where('id', \request("id"))->first();

    // return redirect(route('voyager.contrats.create'));
