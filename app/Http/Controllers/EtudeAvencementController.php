...
use App\Models\EtudeAvancement;
...
class EtudeAvencementController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    public function store(Request $request)
    {
        ...
        $valideE= Perimetre::where('id',$data->perimetre_id)->update(["valideE"=>'oui']);
    ...
    public function destroy(Request $request, $id)
    {
        ...
        foreach ($ids as $id) {
           ...
           $chaine=$ch[1];
            ...
            $valideE= Perimetre::where('id',$data->perimetre_id)->update(["valideE"=>'non']);
            $model = app($dataType->model_name);
            if (!($model && in_array(SoftDeletes::class, class_uses_recursive($model)))) {
    ...
    {        
            $fin = EtudeAvancement::where('id', \request("id"))->first();
            $fin->valide = "oui";
            $mytime = Carbon::now()->format('Y/m/d H:i:s');
            ...
            return redirect(route('voyager.etude-avancements.index'));
    } 
}
