...
class FinancePuitsController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    {
        $fin=FinancePuit::find($id);
    
        $Fin_id= $fin->puit_id;

        $prog = FinancePuit::where('id','=',$id)->first();
        $prog->valide = "non";
        $prog->save();
        $slug = $this->getSlug($request);
        ...
        $db=DB::table('finance_puits')
        ->select('*')
        ->where('id','=',$id)
        ->update(['puit_id' => $Fin_id]);
        // Delete Images
    ...
    {
        $puit = Puit::where('id', \request("id"))->first();
        $slug = $this->getSlug($request);
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable','puit'));
    }
    ...
    {
        $Real = Puit::where('id','=',$request->puit_id)->first();
        $Real->valide = "oui";
        $Real->save();
        $slug = $this->getSlug($request);
        ...
        // $data->update(["puit_id"=>$request->valide]);
        $mytime = Carbon::now()->format('Y/m/d H:i:s');
        $str=$dataType->model_name;
        $ch=str_split($str, 11);
        $chaine=$ch[1];
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $fin=DB::table('finance_puits')->select('puit_id')->where('id','=',$id)->first();
        // dd($fin->puit_id);
        $Real = Puit::where('id','=',$fin->puit_id)->first();
        // dd($Real);
        $Real->valide = "non";
        $Real->save();
    ...
    {        
            $fin = FinancePuit::where('id', \request("id"))->first();
            $fin->valide = "oui";
            $mytime = Carbon::now();
            ...
            return redirect(route('voyager.finance-puits.index'));
    }   
}
